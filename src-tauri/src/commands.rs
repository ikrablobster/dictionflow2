...
pub fn start_microphone_test(state: State<AppState>, device: String) -> Result<(), String> {
    if *state.is_listening.lock().unwrap() {
        return Err("Сначала остановите диктовку".into());
    }
...
pub fn stop_microphone_test(state: State<AppState>) -> Result<(), String> {
    if !*state.is_listening.lock().unwrap() {
        let _ = state.audio.lock().unwrap().stop();
    }
    Ok(())
}
...
fn spawn_live_preview(app: AppHandle, model_size: String, language_mode: String) {
    tauri::async_runtime::spawn(async move {
        // Download/load early so the final result does not pay the full startup cost.
        ...
        loop {
            ...
            let samples = {
                ...
                state.audio.lock().unwrap().snapshot()
            };
...
pub async fn stop_dictation(app: AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    ...
    let cfg = state.config.lock().unwrap().clone();
    let model_path = whisper_engine::ensure_model_downloaded(&cfg.model_size)
    ...
    // The floating overlay is configured as non-focusable, therefore the app
    // the user was typing in remains the insertion target.
